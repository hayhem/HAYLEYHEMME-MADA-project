...
car_smooth <- ggplot(df_sf_fit, aes(long, lat)) +
  ...
  theme(strip.text = element_text(size = 14),
        legend.position = "bottom") +
  labs(title = "Smoothed Map of 
Black-to-White HIV-Related MRR (CAR)
United States, 2020", x = "Ratio", y = "", fill = "") +
  theme(plot.title= element_text(hjust = 0.5))
...
mult_smooth <-ggplot(df_sf_fit, aes(long, lat)) +
  ...
  theme(strip.text = element_text(size = 14),
        legend.position = "bottom") +
  labs(title = "Smoothed Map of 
Black-to-White HIV-Related MRR (GLM)
United States, 2020", x = "Ratio ", y = "", fill = "") +
  theme(plot.title= element_text(hjust = 0.5))

grid_smooth <- grid.arrange(car_smooth, mult_smooth, ncol = 2, nrow = 1)
ggsave(here("results/grid_smooth.png"), grid_smooth, units="in", width=10, height=5, dpi=300)
``` 

Next, let's look at the map for the ratio of predicted to observed (P/O) values. 
```{r}
pred_mult <- ggplot(df_sf_fit, aes(long, lat)) +
...
pred_car <-ggplot(df_sf_fit, aes(long, lat)) +
