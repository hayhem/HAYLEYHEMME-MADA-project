...
Visual assessment and the shapiro test are in agreement that the data is not normally distributed. Let's see if log (x +1) transformation can help resolve this issue.
...
I'll try the Box-Cox transformation first. A limitation of this is that Box Cox cannot have values of zero (or negative numbers). We will have to subset observations greater than zero to continue with the transformation. We will use boxcox() to find lambda. 
`library(MASS)`
```{r}
sex_df_sub_zero <- sex_df %>% 
...
Let's merge back the observations we had to exclude due to limitations of the Box Cox transformation... these will be necessary to carry out spatial analyses..
```{r}
sex_df_ROE <- full_join(sex_df_ROE,sex_df_sub_zero)
