...
```
The Box Cox transformation was not able to normalize the `Standardized Mortality Ratio`. We will move on the Yeo Johnson transformation


We will perform Yeo Johnson transformation on `Death Rate (estimated)` and `Standardized Mortality Ratio`.We will use `yeojohnson` from `BestNormalize` package to extract lambda and `yeo_johnson` from `car` to compute the transformation for each observation.
```{r}
yj_DR <- yeojohnson(sex_df_SMR$`Death Rate (estimated)`)
yj_DR <- yj_DR$lambda
...
plot(lm(sex_df$YJ ~ 1))
```
Unable to normalized the distribution `Death Rate (estimated)` with the Yeo Johnson transformation. 
...
```{r}
saveRDS(sex_df_ROE, here("data/process_data/sex_new.rds"))
```
