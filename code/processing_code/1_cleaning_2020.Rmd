...
Next, we need to estimate the population older than 12. Since I cannot find a source explicitly population older than 12, I will estimate population younger than 13 this using data from Kids Count. 
```{r}
kid_pop <- kid_pop %>% 
...
Let's estimate the number of children under 13 by race
```{r}
kid_race <- kid_race %>%
...
```{r}
rm(deaths, df_pop, kid_pop, pop_sex, pop_less13, us_sex_pop, us_race_pop, new_pop, deaths_sex, race_pop, kid_race, total_pop, deaths_race, late_dx, new_dx, sex_dx, prev, prev_sex, prev_race, v_sup, rec_care)
```
