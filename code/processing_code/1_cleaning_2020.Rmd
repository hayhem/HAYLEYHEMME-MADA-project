...
```{r}
sex_df <- inner_join(deaths_sex, df_pop, by = join_by(State == Location))
```

```{r}
sex_df %>% mutate(M_Rate = `Male Death Cases`/df_pop_male*1e5)
```

Next find expected rate vs observed rate