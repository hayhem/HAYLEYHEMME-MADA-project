...
internet <- read_xlsx(here("data/raw_data/ACSST5Y2020.S2801-2023-04-17T232857.xlsx"), skip = 3, sheet = 2)
```
...
```{r}
internet <- internet %>% 
  select(c(1, "Broadband of any type")) %>% 
  slice(c(-1:-7, -48:-52,-263:-267)) %>% 
  filter(!grepl(",", `Broadband of any type`))
```
...
internet$State<- states 

internet <- internet %>% 
  select(State, `Broadband of any type`) %>% 
  drop_na()

internet$`Broadband of any type` <- gsub("%", "", internet$`Broadband of any type`) 

internet <- internet %>% mutate(`Broadband of any type` = as.numeric(`Broadband of any type`))
```

```{r}
race_df <- full_join(race_df, internet)
sex_df <- full_join(sex_df, internet)
```
